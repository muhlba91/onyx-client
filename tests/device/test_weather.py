...
class TestWeather:
    ...
    def test_update_with_base_device_patch(self, device_mode):
        ...
        # Simulate the case where the API sends a generic Device (UNKNOWN type) with same ID
        update = Device(
            "id",
            "name_from_patch",
            DeviceType.UNKNOWN,
        ...
        weather.update_with(update)
        # Name should be updated; numeric fields must remain intact via getattr fallback
        assert weather.name == "name_from_patch"
        assert weather.device_type == DeviceType.WEATHER
        assert weather.wind_peak == value1
        assert weather.sun_brightness_peak == value2
        assert weather.sun_brightness_sink == value3
